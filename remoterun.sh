...
# copy any needed files
    # cp -r data/es dist/data
    # cp -r data/it dist/data
    # cp -r data/pt dist/data
cp Dockerfile dist
cp build.sh dist
cp run.sh dist
...
ssh $USER@$HOST "~/dist/build.sh $SERVICENAME"

# Run the container in a dettached session
ssh $USER@$HOST "tmux new-session -d -s '$SERVICENAME' ~/dist/run.sh $SERVICENAME"
